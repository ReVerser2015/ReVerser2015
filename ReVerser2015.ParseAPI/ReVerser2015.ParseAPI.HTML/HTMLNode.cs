...
namespace ReVerser2015.ParseAPI.HTML
{
	...
	public partial class HTMLNode
	{
        ...
        /*--------------Added by ReVerser2015 13.06.2015--------------*/

        /// <summary>
        /// Gets the single HTML node with specified 'class' attribute <i>contains</i> specified value.
        /// </summary>
        /// <param name="class">The attribute class to match. May not be <c>null</c>.</param>
        public HTMLNode GetElementByClassName(string className)
        {
            return this.GetElementByClassName(className, false);
        }

        /// <summary>
        /// Gets the single HTML node with specified 'class' attribute contains or equals specified value.
        /// </summary>
        /// <param name="class">The attribute class to match. May not be <c>null</c>.</param>
        /// <param name="strictlyEquals">Indicates whether to find only strictly matches.</param>
        public HTMLNode GetElementByClassName(string className, bool strictlyEquals)
        {
            className = className.Trim();

            if (strictlyEquals)
                return this.SelectNode(".//*[lower-case(@class) = lower-case('" + className + "')]");
            else
                return this.SelectNode(".//*[lower-case(@class) = lower-case('" + className + "') or contains(lower-case(@class), lower-case(' " + className + " ')) or starts-with(lower-case(@class), lower-case('" + className + "')) or ends-with(lower-case(@class), lower-case('" + className + "'))]");
        }

        /// <summary>
        /// Gets the HTML nodes with the 'class' attribute contains specified value.
        /// </summary>
        /// <param name="class">The attribute class to match. May not be <c>null</c>.</param>
        public HTMLNodeCollection GetElementsByClassName(string className)
        {
            return this.GetElementsByClassName(className, false);
        }

        /// <summary>
        /// Gets the HTML nodes with the 'class' attribute contains or equals specified value.
        /// </summary>
        /// <param name="class">The attribute class to match. May not be <c>null</c>.</param>
        /// <param name="strictlyEquals">Indicates whether to find only strictly matches.</param>
        public HTMLNodeCollection GetElementsByClassName(string className, bool strictlyEquals)
        {
            if (strictlyEquals)
                return this.SelectNodes(".//*[@class='" + className + "']");
            else
                return this.SelectNodes(".//*[contains(@class, '" + className + "')]");
        }

        /// <summary>
        /// Gets the single HTML node with specified tag.
        /// </summary>
        /// <param name="class">The tag to match. May not be <c>null</c>.</param>
        public HTMLNode GetElementByTagName(string tag)
        {
            return this.SelectNode(".//" + tag + "");
        }

        /// <summary>
        /// Gets the HTML nodes with specified tag.
        /// </summary>
        /// <param name="class">The tag to match. May not be <c>null</c>.</param>
        public HTMLNodeCollection GetElementsByTagName(string tag)
        {
            return this.SelectNodes(".//" + tag + "");
        }
        ...
        */
		...
		/// <param name="def">The default value to return if not found.</param>
		/// <returns>The value of the attribute if found, the default value if not found.</returns>
		public bool GetAttribute(string name, bool def)
		{
			if (name == null)
			{
				throw new ArgumentNullException("name");
